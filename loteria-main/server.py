...
#Función para generar el tablero de lotería
def generar_tablero(filas, columnas):
        total_celdas = filas * columnas
        #Asegurarse de que el número de celdas no exceda el número de cartas disponibles
        if total_celdas > len(cartas):
                cartas_para_tablero = random.sample(cartas, len(cartas))#Usamos todas las que tenemos y luego rellenamos si necesitamos más
        while len(cartas_para_tablero) < total_celdas:
                ...
                cartas_para_tablero = random.sample(cartas, total_celdas) #Si no, tomamos una muestra aleatoria de las cartas disponibles
        ...
        for r in range (filas):
                ...
                for c in range(columnas):
                        ...
                        fila_actual.append({'color': color, 'carta': cartas_para_tablero[carta_index]})
                        carta_index += 1
